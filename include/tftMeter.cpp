...
void tftMeterUpdate()
{
    if (updateTime <= millis())
    {
        ...
        voltage = 230;
        int i = (voltage / 3);
        plotNeedle(i, 0); // It takes between 2 and 12ms to replot the needle with zero delay
    }
}
...
void analogMeter()
{
    ...
    for (int i = -50; i < 51; i += 5)
    {
        ...
        // Green zone limits
        if (i >= 20 && i < 30) // 210v to 240v
        {
            tft.fillTriangle(x0, y0, x1, y1, x2, y2, TFT_GREEN);
            tft.fillTriangle(x1, y1, x2, y2, x3, y3, TFT_GREEN);
        }

        // Red zone: 150–180 V (i = 0–10) OR >280 V (i ≥ 45)
        if ((i >= 0 && i <= 10) || (i >= 45))
        {
        ...
        if (i % 25 == 0)
        {
            ...
            case -1:
                ...
                break;
            case 0:
                tft.drawCentreString("150", x0, y0 - 7, 2);
                break;
            case 1:
                tft.drawCentreString("225", x0, y0 - 9, 2);
                break;
            case 2:
                tft.drawCentreString("300", x0, y0 - 12, 2);
                break;
            }
        }
    ...
    tft.drawString("Volt", M_SIZE * (5 + 230 - 40), M_SIZE * (119 - 20), 2); // Units at bottom right
    tft.drawCentreString("220 V", M_SIZE * 120, M_SIZE * 70, 4);                // Comment out to avoid font 4
